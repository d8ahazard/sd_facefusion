...
def render() -> None:
    ...
    if job_manager.init_jobs(state_manager.get_item('jobs_path')):
        ...
        JOB_LIST_JOBS_DATAFRAME = gradio.Dataframe(
            headers=job_headers,
            value=job_contents,
            datatype=['str', 'str', 'number', 'str', 'date', 'date', 'str'],
            show_label=False
        )
...
def listen() -> None:
    ...
    if job_list_job_status_checkbox_group:
        job_list_job_status_checkbox_group.change(update_job_dataframe, inputs=job_list_job_status_checkbox_group,
                                                  outputs=JOB_LIST_JOBS_DATAFRAME)
        JOB_LIST_REFRESH_BUTTON.click(update_job_dataframe, inputs=job_list_job_status_checkbox_group,
                                      outputs=JOB_LIST_JOBS_DATAFRAME)
