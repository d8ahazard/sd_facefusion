...
from facefusion.face_helper import create_rotated_matrix_and_size, create_static_anchors, distance_to_bounding_box, \
    distance_to_face_landmark_5, normalize_bounding_box, transform_bounding_box, transform_points
from facefusion.filesystem import resolve_relative_path
from facefusion.thread_helper import thread_semaphore
from facefusion.typing import Angle, BoundingBox, Detection, DownloadSet, FaceLandmark5, ModelSet, Score, \
    VisionFrame
from facefusion.vision import resize_frame_resolution, unpack_resolution
from facefusion.workers.base_worker import BaseWorker


class FaceDetector(BaseWorker):
    MODEL_SET: ModelSet = \
        {
            'retinaface':
                {
    ...
    def collect_model_downloads(self) -> Tuple[DownloadSet, DownloadSet]:
        ...
        if state_manager.get_item('face_detector_model') in ['many', 'retinaface']:
            model_hashes['retinaface'] = self.MODEL_SET.get('retinaface').get('hashes').get('retinaface')
            model_sources['retinaface'] = self.MODEL_SET.get('retinaface').get('sources').get('retinaface')
    ...
    def detect_rotated_faces(self, vision_frame: VisionFrame, angle: Angle) -> Tuple[
        ...
        bounding_boxes, face_scores, face_landmarks_5 = self.detect_faces(rotated_vision_frame)
        bounding_boxes = [transform_bounding_box(bounding_box, rotated_inverse_matrix) for bounding_box in
                          bounding_boxes]
        face_landmarks_5 = [transform_points(face_landmark_5, rotated_inverse_matrix) for face_landmark_5 in
                            face_landmarks_5]
    ...
    def detect_with_scrfd(self, vision_frame: VisionFrame, face_detector_size: str) -> Tuple[
        List[BoundingBox], List[Score], List[FaceLandmark5]]:
        bounding_boxes = []
        face_scores = []
        face_landmarks_5 = []
    ...
    def detect_with_yoloface(self, vision_frame: VisionFrame, face_detector_size: str) -> Tuple[
        List[BoundingBox], List[Score], List[FaceLandmark5]]:
        bounding_boxes = []
        face_scores = []
        face_landmarks_5 = []
    ...
    def forward_with_scrfd(self, detect_vision_frame: VisionFrame) -> Detection:
        face_detector = self.get_inference_pool().get('scrfd')
