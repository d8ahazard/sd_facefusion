...
# Custom masks have been replaced by auto-padding system
            # Show auto-padding detection results if available
            if hasattr(target_face, 'auto_padding_data') and target_face.auto_padding_data['padding_needed']:
                # Create a simple mask to visualize auto-padding detection
                auto_padding_mask = numpy.ones(crop_vision_frame.shape[:2], dtype=numpy.float32) * 0.3
                masks.append(("auto-padding", auto_padding_mask))
